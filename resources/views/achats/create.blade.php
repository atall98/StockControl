...
<div class="container mt-5">
    ...
    <form action="{{ route('achats.store') }}" method="POST">
        <h1 class="mb-4 text-center">Créer un achat</h1>
        @csrf
        <div class="form-group">
            <label for="fournisseur_id" class="col-sm-3 col-form-label font-weight-bold">Fournisseur</label>
            <div class="col-sm-9">
                <select name="fournisseur_id" class="form-control form-control-lg" required>
                    <option value="" disabled selected>Sélectionner un fournisseur</option>
                    @foreach ($fournisseurs as $fournisseur)
                        <option value="{{ $fournisseur->id }}">{{ $fournisseur->nom_fournisseurs }}</option>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <table class="table table-bordered">
                <thead class="thead-dark">
                    ...
                    <tr>
                        ...
                        <th><input type="text" name="nom_societe" class="form-control" placeholder="Nom de la société" required></th>
                    </tr>
                </thead>
                <thead>
                ...
                <tbody id="produit-fields">
                    <tr class="produit-row">
                        ...
                        <td><button type="button" class="btn btn-danger btn-sm" onclick="removeProduitRow(this)">Supprimer</button></td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="form-group row">
            <label class="col-sm-3 col-form-label font-weight-bold">Total achat :</label>
            <div class="col-sm-9">
                <span id="total-achat" class="font-weight-bold text-primary" style="font-size: 1.2rem;">0.00</span>
            </div>
        </div>

        <button type="submit" class="btn btn-primary btn-lg btn-block">Valider l'achat</button>
    </form>
</div>
...
<script>
    function fetchProduitInfo(input) {
        ...
        const prixTotalSpan = row.querySelector('.prix-total');
        ...
        fetch(`/produits/search/${value}`)
            ...
            .then(data => {
                ...
                prixUnitaireSpan.innerText = parseFloat(data.prix_achat).toFixed(2);
    ...
    function updateTotal() {
        ...
        document.getElementById('total-achat').innerText = total.toFixed(2) + ' ';
    }
